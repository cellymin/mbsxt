...
class ManageZxController extends \Component\AdminController
{
    ...
    {
        $canshu = i('request.');
        $zx_ID = intval($canshu['zid']);
        $beizhu = trim($canshu['beizhu']);
        $Managezx = m('managezx');
        $Application = m('application');
        $picturestr = array();
        $info = $Managezx->where('zx_ID=' . $zx_ID)->find();
        if (empty($info)) {
            $data['code'] = 0;
            $data['msg'] = '无效的操作！';
        ...
        $userID = $info['userID'];
        $ifdaozhen = $Managezx->where('zx_ID=' . $zx_ID . ' and userID=' . $userID . ' and daozhen_time is not null')->find();
//		echo M()->getLastSql();die();
        ...
        $ifhas = $Application->where('zx_ID=' . $zx_ID . ' and userID=' . $userID . ' and (status=1 or status=2)')->find();
        if (!empty($ifhas)) {
            if (intval($ifhas['status']) == 1) {
                $data['msg'] = '已提交,不要重复提交！';
            } else if (intval($ifhas['status']) == 2) {
                $data['msg'] = '已审核,不要重复提交！';
            }
            $data['code'] = 0;
            exit(json_encode($data));
        ...
        if ($_FILES) {
            ...
            foreach ($_FILES["upfile"]["error"] as $key => $error) {
                if ($error == UPLOAD_ERR_OK) {
                    $re = $this->upload_multi($uploaddir, $_FILES["upfile"], $i, $userID, $zx_ID);
                    if (!empty($re)) {
                        $picturestr[] = $re;
                    }
                }
        ...
        foreach ($picturestr as $k => $v) {
            $picinfo = explode('_', $v);
            if (!file_exists($_SERVER['DOCUMENT_ROOT'] . "/upload/" . date('Ymd', $picinfo[2]) . '/' . $v)) {
                unset($picturestr[$k]);
            }
        }
        if (!empty($picturestr)) {
        ...
        //申请保留单创建
        $Application->zx_ID = $zx_ID;
        $Application->yy_ID = $info['yy_ID'];
        $Application->zxfs_ID = $info['zxfs_ID'];
        $Application->userID = $info['userID'];
        $Application->bz_ID = $info['bz_ID'];
        $Application->xx_ID = $info['xx_ID'];
        $Application->zx_time = $info['zx_time'];
        $Application->apply_time = date('Y-m-d H:i:s', time());
        $Application->status = 1;
        $Application->beizhu = $beizhu;
        $Application->appsrc = $picturestrs;
        try {
            m()->startTrans();
            $re = $Application->add();
            $mandata['shifoubaoliu'] = 2;
            $re1 = $Managezx->where('zx_ID=' . $zx_ID . ' and userID=' . $userID)->save($mandata);
            if ($re && $re1) {
                m()->commit();
                $data['code'] = 200;
                $data['msg'] = '提交成功！';
    ...
    public function upload_multi($path, $photo, $i, $userid, $zx_id)
    {
        $date = date('Ymd', time());
        ...
        $uploadfile = $path . '/' . $date . '/' . $userid . '_' . $zx_id . '_' . time() . '_' . $i . '.' . $piece[1];
    ...
    {
        //保留申请单
        //检索 1状态 2申请时间 3咨询时间 4客服
        //显示超时几天
        $canshu = i('request.');
        $yyid = intval($canshu['yy_ID']);
        $appstatus = intval($canshu['appstatus']);
        $kefu = $canshu['kefu'];
        $shouji = $canshu['shouji'];
        $zixun_timeStart = $canshu['zixun_timeStart'];
        $zixun_timeEnd = $canshu['zixun_timeEnd'];
        $apply_timeStart = $canshu['apply_timeStart'];
        $apply_timeEnd = $canshu['apply_timeEnd'];
        $uid = intval($_SESSION['username_lf']);
        $role = intval($_SESSION['user_role']);
        $where = '';
        if($yyid){
            $where .= ' and a.yy_ID='.$yyid;
            $this->assign('dqyyID', $yyid);
        }
        if(!empty($appstatus)){
            $where .= ' and a.status='.$appstatus;
            $this->assign('appstatus', $appstatus);
        }
        if(!empty($kefu)){
            $where .= ' and a.userID='.$kefu;
            $this->assign('dqzxy', $kefu);
        }
        if(!empty($shouji)){
            $where .= ' and  m.shouji like \'%' . trim($shouji) . '%\' ';
            $this->assign('serchshouji', $shouji);
        }
        if(!empty($zixun_timeStart)){
            $where .= ' and  m.zx_time > \'' . trim($zixun_timeStart) . '\' ';
            $this->assign('zixun_timeStart', $zixun_timeStart);
        }
        if(!empty($zixun_timeEnd)){
            $where .= ' and  m.zx_time > \'' . trim($zixun_timeEnd) . '\' ';
            $this->assign('zixun_timeEnd', $zixun_timeEnd);
        }
        if(!empty($apply_timeStart)){
            $where .= ' and  a.apply_time > \'' . trim($apply_timeStart) . '\' ';
            $this->assign('apply_timeStart', $apply_timeStart);
        }
        if(!empty($apply_timeEnd)){
            $where .= ' and  a.apply_time > \'' . trim($apply_timeEnd) . '\' ';
            $this->assign('apply_timeEnd', $apply_timeEnd);
        }
        $user = m('useradmin');
        $ssyy = array();
        $yyarr = array();
...
//        echo '<pre/>';var_dump($zixunyuan1);die();
        $this->assign('ssyy', $ssyy);
        $this->assign('countyy', count($ssyy));
        ...
        if ($role == 1) {
            //超级管理员 查看所有的保留申请
            $appinfo = $Application->field('a.*,m.huanzeName,m.shouji,u.userchinaname')->alias('a')->join('left join oa_useradmin u on a.userID=u.user_ID')->join('left join oa_managezx m on a.zx_ID=m.zx_ID')->where('a.status!=0'.$where)->select();
        } else if ($role == 5 || $role == 4) {
            //咨询 或咨询组长 查看自己的保留申请
            $appinfo = $Application->field('a.*,m.huanzeName,m.shouji,u.userchinaname')->alias('a')->join('left join oa_useradmin u on a.userID=u.user_ID')->join('left join oa_managezx m on a.zx_ID=m.zx_ID')->where('a.userID=' . $uid . ' and a.status!=0'.$where)->select();
        }
//		echo M()->getLastSql();die();
    ...
    public function changeapply()
    {
        //修改申请单状态
        $canshu = i('post.');
        $zx_ID = intval($canshu['zx_id']);
        $appid = intval($canshu['appid']);
        $managezx = m('managezx');
        $application = m('application');
        $info = $managezx->where('zx_ID=' . $zx_ID)->find();
        if (empty($info)) {
        ...
        $ifhas = $application->where('zx_ID=' . $zx_ID . ' and userID=' . $userID . ' and (status=1 or status=2)')->find();
        if (!empty($userID)) {
            if (intval($ifhas['status']) == 2) {
                $data['msg'] = '已审核,不要重复审核！';
                $data['code'] = 0;
                exit(json_encode($data));
            } else if (intval($ifhas['status']) == 3) {
                $data['msg'] = '已审核,不要重复审核！';
                $data['code'] = 0;
                exit(json_encode($data));
            }
        ...
        if (isset($canshu['sign']) && !empty($canshu['sign']) && intval($canshu['sign']) == 2) {
            //驳回
            $data['shifoubaoliu'] = 0;
            try {
                m()->startTrans();
                $re = $managezx->where('zx_ID=' . $zx_ID)->save($data);
                $result = trim($canshu['shuoming']);
                $appdata['fback_time'] = date('Y-m-d H:i:s');
                $appdata['status'] = 3;
                $appdata['result'] = $result;
                $re1 = $application->where('apply_id=' . $appid)->save($appdata);
                if ($re && $re1) {
                    m()->commit();
                    unset($data);
                    $data['code'] = 200;
            ...
            //通过
            $data['shifoubaoliu'] = 1;
            $appinfo = $application->where('apply_id=' . $appid)->find();
            if (empty($appinfo)) {
                unset($data);
                $data['code'] = 0;
                $data['msg'] = '审核失败';
            ...
            $zx_time = strtotime($appinfo['zx_time']);
            $app_end_time = strtotime('+1 month', $zx_time);
            $now = time();
            if ($now > $app_end_time) {
                $appdata['fback_time'] = date('Y-m-d H:i:s', $app_end_time);
            } else {
                $appdata['fback_time'] = date('Y-m-d H:i:s');
            }
            $appdata['status'] = 2;
    ...
    public function fenpeikefu()
    {
        $canshu = i('post.');
        $zxids = $canshu['zxids'];
        $cxfpid = intval($canshu['cxfpid']);
        $managezx = m('managezx');
        $huanze = m('huanze');
        $log = m('log');
        $user = m('useradmin');
        $application = m('application');
        $data = array();
        $zxidarr = explode(',', $zxids);
        $re1 = $managezx->where('zx_ID in (' . $zxids . ')')->group('userID,daozhen_time,shifoubaoliu')->select();
        $username = $user->where('user_ID=' . $cxfpid)->find();
        $caozuoname = $user->field('userchinaname')->where('user_ID=' . $_SESSION['username_lf'])->find();
        if (empty($re1)) {
            $data['code'] = 0;
            $data['msg'] = '操作为空，请核实';
            exit(json_encode($data));
            ...
            $data['code'] = 0;
            $data['msg'] = '分配条件不符合，请核实';
            exit(json_encode($data));
        } else {
            $olduser = $re1[0]['userchinaname'];
            $newuser = $username['userchinaname'];
            $caozuouser = $caozuoname['userchinaname'];
            if ($re1[0]['userID'] == $cxfpid) {
                $data['code'] = 0;
                $data['msg'] = '分配客服为原客服！';
                exit(json_encode($data));
            }
            try {
                m()->startTrans();
                //修改患者表
                $hzdata['userID'] = $cxfpid;
                $re = $huanze->where('zx_ID in (' . $zxids . ')')->save($hzdata);
                //修改咨询表
                $mzdata['userID'] = $cxfpid;
                $re2 = $managezx->where('zx_ID in (' . $zxids . ')')->save($mzdata);

                //添加日志
                $logdata['zx_ID'] = $zxids;
                $logdata['userID'] = intval($_SESSION['username_lf']);
                $logdata['type'] = 1;
                $logdata['intro'] = $caozuouser . '将' . $olduser . '客户' . $zxids . '分配给' . $newuser . '数量' . count($zxidarr);
                $logdata['created_time'] = date('Y-m-d H:i:s');
                $re3 = $log->add($logdata);
                if ($re && $re2 && $re3) {
                    m()->commit();
                    $data['code'] = 200;
                    $data['msg'] = '分配成功';
                    $data['num'] = count($zxidarr);
                    exit(json_encode($data));
                }
            } catch (\Exception $e) {
                m()->rollback();
    ...
    {
        $arr = array('1' => '编号', '4' => '所属门店', '5' => '患者姓名', '6' => '预约日期', '7' => '联系方式', '8' => '咨询方式', '9' => '咨询病种', '10' => '来源渠道', '11' => '搜索关键词', '12' => '到诊时间', '13' => '客服', '14' => '回访次数', '15' => '消费', /* '16' => '住院', */
            '17' => '到诊状态', '19' => '预约备注', '20' => '性别', '21' => '年龄', '22' => '预约号', '23' => '匹配关键词', '24' => 'QQ号', '25' => '微信号', '26' => '最后回访');
        return $arr;
    }
